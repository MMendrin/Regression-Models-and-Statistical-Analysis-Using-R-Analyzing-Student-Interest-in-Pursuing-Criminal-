...
library(ltm)
cronbach.alpha(all_questions)

cronbach.alpha(projectdata %>% dplyr::select(Acc1:Acc2) %>% drop_na())
?cronbach.alpha
...
# FACTOR ANALYSIS USING FA() FUNCTION FROM PSYCH PACKAGE

# MODEL 1

library(psych)
output <- fa(apls_items, nfactors = 2)
output$loadings # factor loadings
